...
- **Lock-screen card disappeared after updates.** Android cancels an app's notifications on update/reinstall (and users can "clear all"), while the app only remembered the on/off flag. `ensureLockScreenCardPinned()` now runs on launch and whenever the app returns to the foreground: if the flag is on and the card is not among the presented notifications, it fetches the current card and pins it again.
- **10+ second requests = Render free-tier cold start** (instance sleeps after 15 idle minutes). Three-part fix: `.github/workflows/keepalive.yml` pings `/health` every 10 minutes (one always-on service fits the 750 free instance-hours), the app pings `/health` the moment it opens, and if that ping takes more than 3 s a snackbar says the server is waking up instead of leaving a bare spinner.
- **Silent Save on an invalid date of birth.** The error text sits under the field, far above the Save button. Save now shows a snackbar naming the field and the reason and scrolls the field into view (`Screen` exposes its ScrollView ref); the document review form gives the same message for bad dates.
- **Samsung lock screens show notifications as icons only by default.** With the card pinned, the lock screen displays just the app icon in the row under the clock until the phone's setting "Lock screen → Notifications" is switched to show content ("Details"). Android offers no API to change that from an app, so the lock-screen screen now says so. Verified on the Samsung test phone: the re-pin on launch/resume restores the card after every app update (confirmed with `dumpsys notification`), and the Save-time feedback scrolls to the invalid date-of-birth field with a snackbar naming it.
